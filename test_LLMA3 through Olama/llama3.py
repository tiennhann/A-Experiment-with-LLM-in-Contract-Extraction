...
def generate_contract_details(contract):
    ...
    prompt = f"""
        ...
        3/ Categories/grade of the product/service if we have
...
def get_clauses(contract_details, contract_text):
    ...
    prompt = f"""
        ...
        4) <deadline>: The time line or schedule for fulfilling the responsibility. The deadline should be specified in terms of frequency (monthly, weekly, e.g) or a specific frequency (e.g, monthly, weekly). Or, a specific week number (e.g, by_week <week_number>).where one month is considered as 4 weeks. If the contract specifies a monthly deadline, convert it into by_week to maintain the consistency.
...
# Load the JSON file
with open(os.path.join(BASE_DIR, 'data', 'dev_data.json'), 'r') as file:
...
with open(os.path.join(BASE_DIR, 'output', 'output_json.json'), 'w') as outfile:
